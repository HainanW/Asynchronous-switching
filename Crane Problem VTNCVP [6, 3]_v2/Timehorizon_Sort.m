...
% u1   # 1-6 % Nj(1)
% u2   # 7-9 % Nj(1)+1 : sum(Nj)
% t_u1 # 10-14 % sum(Nj)+1 : sum(Nj)+N1-1
% t_u2 # 15-16 % sum(Nj)+N1: 2*sum(Nj)-nu
% tF   # 17    % 
% m.Nj = [6,3];
% m.nu = 2; % number of control vars
% m.nx = 7; % number of original state vars
% m.p = 7;
...
%-------------------------------------------------------------------------
% 2. tspan for all controls and time intervals
% 2-1 theta_u1/u2: theta for u1&2
% u1_tspan = [0 tvec_u1 tF];
% u2_tspan = [0 tvec_u2 tF];

% theta_u1 = zeros(1,m.Nj(1)); 
% for i = 1 : m.Nj(1)
%     theta_u1(i) = u1_tspan(i+1) - u1_tspan(i);
% end
% 
% theta_u2 = zeros(1,m.Nj(2)); 
% for i = 1 : m.Nj(2)
%     theta_u2(i) = u2_tspan(i+1) - u2_tspan(i);
% end

% 2-2 theta_span = [theta1 ~ theta8]