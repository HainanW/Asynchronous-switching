...
Nj = m.Nj; % [10,5] % # of time stages 
nx = m.nx; % 7
nu = m.nu; % 2; % nu = 2
m1 = m.m1; % 2; 
P = m.p ; % P = 13
nx_t = nx + m1 + (nx+m1)*(P + 1);% 9+ 9*14=135
...
for i = 1 : Nj(2) % 1 : 5
    ...
    pupw = zeros(nu+1, Nj(2)); % !3x5
    pupw(2, i) = 1;
    res_phi_u2 = pfpx_S_phi_u2 + pfpu*pupw; % 9x5
...
for i = 1 : P+1 % 1 : 14
    ...
    pupw = zeros(nu+1, P+1);
    pupw(3, i) = 1;
    res_psi = pfpx_S_psi + pfpu*pupw; % 9x14
